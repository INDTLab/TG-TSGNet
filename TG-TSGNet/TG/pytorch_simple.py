...
from TG import distributed_utils
from TG.vae import OpenAIDiscreteVAE, VQGanVAE
from TG.transformer_simple import Transformer, DivideMax, Mambablock
# from dalle_pytorch.transformer718 import Transformer, DivideMax, Mambablock
...
class DALLE(nn.Module):
    ...
    def generate_texts(
        ...
        *,
        ...
        for _ in range(text_tokens.shape[1], text_seq_len):
            ...
            # out = self.mamba(output_transf)
            # out = self.transformer(tokens)
            # out = self.mamba(out)
            # out = self.transformer(out)
            
            out = self.transformer1(tokens)
            out = self.mamba(out)
    ...
    def forward(
        ...
        out = self.transformer1(tokens, cache=cache)
        out = self.mamba(out)
        # out = self.transformer2(out)
        
        
        
        # 这里尝试仅用mamba处理img的那块序列
        ...
        # text_out = self.mamba(text_seq)
        # out = torch.cat((text_out, img_seq), dim=1)
        
        # out = self.mamba(out)
        # out = self.transformer(out, cache=cache)
        ...
        # img_seq_mamba = self.mamba(img_seq)
        
        # print("img_seq_mamba shape:", img_seq_mamba.shape)
        # print("img_seq_mamba:", img_seq_mamba)
        
        
        #tokens = torch.cat((text_seq, img_seq), dim=1)
        ...
        # print(f"Image Loss: {loss_img.item()}")
