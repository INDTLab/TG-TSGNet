...
class ClipTokenizer:
    ...
    def tokenize(self, texts, context_length=77, truncate_text=False):
        ...
        all_tokens = torch.empty((len(texts), context_length), dtype=torch.long)

        for i, text in enumerate(texts):
            if len(text.split()) > context_length:
                text = ' '.join(text.split()[:context_length])
            ...
            all_tokens[i, :tokens.shape[1]] = tokens
