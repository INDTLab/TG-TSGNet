...
class MHAttention(nn.Module):
    def __init__(self, dim, seq_len, causal=True, num_heads=8, dim_head=64, dropout=0., stable=False,
                 static_mask=None):
        super().__init__()
...
def get_none_linear_projection(query_dim, mid_dim=None):
    return (
        nn.Sequential(
            nn.Linear(query_dim, mid_dim or query_dim),
            nn.LayerNorm(mid_dim or query_dim),
            nn.LeakyReLU(inplace=True),
            nn.Linear(mid_dim or query_dim, query_dim),
        )
        if mid_dim != -1
        else nn.Identity()
    )
