...
class SRImplicitDownsampledFast(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.inp_size is None:
            ...
            h_hr = round(h_lr * s)
            w_hr = round(w_lr * s)
            if h_hr > img.shape[-2] or w_hr > img.shape[-1]:
                h_hr = min(img.shape[-2], round(h_lr * s))
                w_hr = min(img.shape[-1], round(w_lr * s))
            

            if h_hr > img.shape[-2] or w_hr > img.shape[-1]:
                raise ValueError(f"Generated crop size ({h_hr}, {w_hr}) exceeds input size ({img.shape[-2]}, {img.shape[-1]})")
