...
# vmamba_2我想试着改变右侧分支z那，本来右侧分支只是多分出来一块走了个silu的激活函数，我想改成1x1卷积来多提取局部信息试一试
class SS2D(nn.Module):
    def __init__(
        ...
        # 添加了一个右侧分支 替换silu
        self.v = nn.Conv2d(self.d_inner, self.d_inner, 1)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        xz = self.in_proj(x)
        # 将投影后的张量在最后一个维度（通道维度）上切分为 x 和 z，每个的形状为 (B, H, W, d_inner)
        # z是走右侧的那个分支
        x, z = xz.chunk(2, dim=-1)  # (b, h, w, d)
        # x 的形状是 (B, H, W, C//2)
        # z 的形状也是 (B, H, W, C//2)

        #  x 的维度从 (B, H, W, C//2) 转换为 (B, C//2, H, W)
        ...
        # 这一块是我的离谱改动
        # 将 z 的维度转换为 (B, d, H, W)，与 y 的维度 (B, d, H, W) 进行逐元素相乘
        z = z.permute(0, 3, 1, 2).contiguous()  # (B, d, H, W)
        # 进行逐元素乘法
        y = y.permute(0, 3, 1, 2)  # (B, d, H, W)
        y = y * self.v(z)  # (B, d, H, W) 与 (B, C//2, H, W) 相乘
        # 再将 y 的顺序变回 (B, H, W, d)
