...
"""
# 这个DE卷积是包括了五个并行部署的卷积层
# 包括普通卷积 中心差分卷积 角度差分卷积 水平差分卷积 垂直差分卷积
# 输入尺寸为(batch_size, channels, height, width)


# 中心差分卷积 (Center-Difference Convolution)
# 从标准卷积权重中取出一个3×3的卷积核权重。
# 将中心像素位置的权重（索引为4的位置）替换为中心像素与周围像素权重的差值，
# 即 conv_weight_cd[:, :, 4] = conv_weight[:, :, 4] - conv_weight[:, :, :].sum(2)
# 这个操作使得卷积核在进行卷积时能够突出输入特征中中心像素与周围像素的变化。
