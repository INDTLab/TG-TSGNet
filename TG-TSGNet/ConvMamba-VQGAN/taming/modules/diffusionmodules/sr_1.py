...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.norm_out_1 = Normalize(ch*2)
        self.conv_out_1 = torch.nn.Conv2d(ch*2,
                                        out_ch,
                                        kernel_size=3,
                                        stride=1,
                                        padding=1)
    ...
    def forward(self, z):
        ...
        # 再最后一次上采样之前，把图片输出看一眼
        h_1 = self.one.block_3(h)
        h_1 = self.one.block_3(h_1)
        h_1 = self.one.block_4(h_1)
        ...
        h_1 = self.norm_out_1(h_1)
        h_1 = nonlinearity(h_1)
        h_1 = self.conv_out_1(h_1)
