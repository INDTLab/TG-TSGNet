...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=32, bias=False):
...
class NTB(nn.Module):
    ...
    def __init__(
        ...
        #self.e_mhsa = E_MHSA(in_channels//2, head_dim=head_dim, sr_ratio=sr_ratio, attn_drop=attn_drop, proj_drop=drop)
        
        self.attn = Attention(in_channels//2)
        self.mhsa_path_dropout = DropPath(path_dropout)

        self.projection = PatchEmbed(in_channels, in_channels//2, stride=1)
        #self.v2 = InvertedResidual(in_channels=in_channels//2, out_channels=in_channels//2, stride=1, expand_ratio=2)
        #self.mhca = MHCA(in_channels//2, head_dim=head_dim)
