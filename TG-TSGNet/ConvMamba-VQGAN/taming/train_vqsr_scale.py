...
class srvq(pl.LightningModule):
    def __init__(self, sr_model: SRModel, vq_model: VQModel, sr_weight=1.0, vq_weight=1.0):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            scaled_sr_loss = loss * self.sr_weight
            
            # print("SR Model Loss:", loss)
            
            self.log("sr_train_loss", loss, prog_bar=True)
            
            self.log("scaled_sr_loss", scaled_sr_loss, prog_bar=True)
            
            return scaled_sr_loss
            ...
            scaled_vq_loss = loss * self.vq_weight
            
            # print("VQ Model Loss:", loss)
            
            self.log("vq_train_loss_ae", loss, prog_bar=True)
            self.log(f"scaled_vq_ae_loss", scaled_vq_loss, prog_bar=True)
            
                
            return scaled_vq_loss
            ...
            self.log(f"scaled_vq_disc_loss", scaled_vq_loss, prog_bar=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        sr_loss_scaled = sr_val_output['val_loss'] * self.sr_weight
        vq_rec_loss_scaled = vq_val_output["rec_loss"] * self.vq_weight
        

        self.log("sr_val_loss", sr_val_output['val_loss'], prog_bar=True)
        self.log("vq_val_rec_loss", vq_val_output["rec_loss"], prog_bar=True)
        self.log("scaled_sr_val_loss", sr_loss_scaled, prog_bar=False)
        self.log("scaled_vq_val_rec_loss", vq_rec_loss_scaled, prog_bar=False)
...
if __name__ == "__main__":
    ...
    try:
        ...
        joint_model = srvq(sr_model, model, sr_weight=0.8, vq_weight=1.0)
